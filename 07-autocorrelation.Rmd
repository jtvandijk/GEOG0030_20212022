...
### Getis-Ord General G
The Getis-Ord General G test tells us whether high or low values are clustering. For Getis-Ord, as it is **recommended** to use a proximity-based neighbour matrix, we will use our Fixed Distance definition:
...
We can see we actually on have two cluster types present within our data set - alongside the `No Significance` value. We therefore need to ensure our palette only includes two colours for these two cluster types, plus a white colour for `No Significance`:

```{r 07-plot-LMI-hscs, warnings=FALSE, message=FALSE, cache=TRUE}
# create a colour palette
# White (No Significance), Light Blue (Low-High) and Dark Red (High-High)
pal <- c("#F5F5F5", '#91bfdb', '#67001f')

# plot the clusters
tm_shape(obesity_ward_sdf) +
  tm_polygons(col = "cluster_type", palette = rev(pal), title = "Cluster Type") +
