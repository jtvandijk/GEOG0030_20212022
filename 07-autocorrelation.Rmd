...
### Getis-Ord General G
The Getis-Ord General G test tells us whether high or low values are clustering. For Getis-Ord, as it is **recommended** to use a proximity-based neighbour matrix, so we will use our Fixed Distance definition:

```{r 07-run-GO, warnings=FALSE, message=FALSE, cache=FALSE, tidy=TRUE}
...
```{r 07-map-LMI-p, warnings=FALSE, message=FALSE, cache=TRUE, tidy=TRUE}
# significance breaks
...
```{r 07-LMI-cluster-type, warnings=FALSE, message=FALSE, cache=FALSE, tidy=TRUE}
...
# obesity_ward_sdf <- obesity_ward_sdf %>% mutate(cluster_type= LMI_diff)
...
#   tm_layout(main.title = "Cluster Map of Childhood Obesity in London in 2014", 
