...
You can access the Lecturecast recording here: [[Link]](https://echo360.org.uk/section/9991b6b4-5c6b-4edd-aa84-7ff49b1170e3/home). The slides for this week's lecture can be downloaded here: [[Link]](https://github.com/jtvandijk/GEOG0030/tree/master/data/ppt/w09_geo.pdf).
...
In addition to those libraries you should now be familiar with, we will need to install `gstat`. After installing `gstat`, within your script, add the following libraries for loading:
...
:::note
**Note** <br />
Some of the following code will unfortunately not work with some recent versions of the `rgdal` package. This is because the `gstat` package that both our IDW function is from has not managed to yet accommodate the changes `rgdal` has made to using `GDAL 3` and `PROJ 6`. if the `gsat` below does not work for you, there is an additional section of code that uses the `spatstat` library to conduct the IDW analysis. 
:::
